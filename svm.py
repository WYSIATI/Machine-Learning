...
# note: requires cvxopt from cvxopt.org
...
class svm:
	...
	def train_svm(self,X,targets):
		...
		sol = cvxopt.solvers.qp(cvxopt.matrix(P),cvxopt.matrix(q),cvxopt.matrix(G),cvxopt.matrix(h), cvxopt.matrix(A.copy()), cvxopt.matrix(b))
